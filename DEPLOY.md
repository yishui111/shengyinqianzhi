...
2. 处理选项**默认全部不勾选**：什么都不勾 = 只把视频/音频原样转成 44.1k 单声道 wav（纯转码，适合方言录音/干净素材）；需要去 BGM 时勾「人声分离」、去混响勾「去混响」、去其他说话人勾「只保留主要说话人」、去掌声勾「去掌声/欢呼/音效」、响度归一化/生成训练切片按需勾选；
...
| 数据目录 | `ziliao\input\`（上传的原始素材，按任务时间分目录）、`ziliao\output\<任务时间>\`（结果 wav）、`ziliao\output\merged\`（「合成一个音频」结果）、`ziliao\asr\<时间戳>\`（语音转文字 txt 文档） |
